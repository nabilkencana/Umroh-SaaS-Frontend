...
export default function LengkapiDataPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Upload files to Cloudinary (optional - skip if Cloudinary not configured)
            let ktpUrl = '';
            let passportUrl = '';
            let vaccineUrl = '';
            ...
            console.log('📊 All jamaah count:', jamaahList.length);
            console.log('📧 Looking for email:', formData.email);

            const existingJamaah = jamaahList.find((j) => j.email === formData.email);
            console.log('🔍 Existing jamaah found:', existingJamaah ? 'YES' : 'NO');
            if (existingJamaah) {
                console.log('📝 Existing jamaah ID:', existingJamaah.id);
            }
            ...
            console.log('💾 Data to save:', {
                ...
                package_type: jamaahData.package_type,
                room_type: jamaahData.room_type,
                total_price: jamaahData.total_price,
            });

            let result;
            if (existingJamaah) {
                // Update existing jamaah
                console.log('🔄 Updating existing jamaah...');
                result = await jamaahService.update(existingJamaah.id, jamaahData);
                console.log('✅ Update result:', result);
            } else {
                // Create new jamaah
                console.log('➕ Creating new jamaah...');
                result = await jamaahService.create(jamaahData);
                console.log('✅ Create result:', result);
            }

            alert('Data berhasil dilengkapi! Silakan tunggu konfirmasi dari admin.');

            // Reload jamaah data in profile
            console.log('🔄 Redirecting to profile with refresh...');
            router.push('/dashboard/jamaah-profile?refresh=true');
        } catch (err: any) {
        ...
        <div className="min-h-screen bg-gray-50 py-8">
            <div className="max-w-3xl mx-auto px-4">
                ...
                <div className="bg-white rounded-lg shadow-md p-8">
                    <h1 className="text-3xl font-bold text-gray-800 mb-2">Lengkapi Data Jamaah</h1>
                    <p className="text-gray-600 mb-8">Lengkapi data Anda untuk pendaftaran umroh</p>
